...
window.renderChatMessages = (chatId) => {
    const chatBox = document.getElementById('chat-messages') || document.getElementById('chat-messages-user');
    if (!chatBox) return;

    const messages = State.messages[chatId] || [];
    const currentUser = State.user ? State.user.name : (State.view === 'admin' ? 'Admin' : 'Guest');

    chatBox.innerHTML = messages.map(m => {
        const isMe = m.sender === currentUser;
        const bg = isMe ? (State.view === 'admin' ? '#1a2a3a' : '#138808') : 'white';
        const color = isMe ? 'white' : '#1a2a3a';
        const align = isMe ? 'flex-end' : 'flex-start';
        const radius = isMe ? '15px 15px 0 15px' : '15px 15px 15px 0';
        const shadow = isMe ? '0 2px 5px rgba(0,0,0,0.1)' : '0 2px 5px rgba(0,0,0,0.05)';

        return `
            <div style="align-self:${align}; background:${bg}; color:${color}; padding:10px 15px; border-radius:${radius}; max-width:80%; box-shadow:${shadow}; font-size:0.9rem;">
                ${m.text}
    ...
    if (messages.length === 0) {
        chatBox.innerHTML = `<div style="text-align:center; color:#999; font-size:0.8rem; margin-top:20px;">No messages yet. Start a conversation!</div>`;
    }
...
function saveGlobalData() {
    ...
    saveToFirebase();
    return Promise.resolve(); // Resolve immediately for 'fast' feel
}
...
function renderHome(container) {
    ...
    container.innerHTML = `
        ...
        <div style="padding: 0 12px; margin-bottom: 20px; position: relative; z-index: 10; display: flex; gap: 8px; align-items: center;">
            ...
            <div onclick="openMessenger()" style="height: 50px; padding: 0 15px; background: #138808; border-radius: 50px; display: flex; align-items: center; gap: 6px; cursor: pointer; flex-shrink: 0; box-shadow: 0 10px 20px rgba(19, 136, 8, 0.25); border: 2.5px solid white;">
    ...
    modal.innerHTML = `
        <div class="modal-content scale-in" style="max-width:400px; padding:0; border-radius:20px; overflow:hidden; border:none; background:#f0f2f5;">
            ...
            <div style="background:linear-gradient(135deg, #FF9933, #138808); padding:15px 20px; color:white; display:flex; justify-content:space-between; align-items:center;">
                ...
                <i class="fas fa-times" onclick="State.activeChatId=null; closeModal()" style="cursor:pointer; font-size:1.2rem;"></i>
            </div>
            ...
            <div style="background:white; padding:15px; display:flex; gap:10px; align-items:center; border-top:1px solid #eee;">
                ...
                <div onclick="sendMessageToAdmin()" style="width:40px; height:40px; background:#138808; border-radius:50%; display:flex; align-items:center; justify-content:center; color:white; cursor:pointer; box-shadow:0 4px 10px rgba(19, 136, 8, 0.2);">
    ...
    modal.innerHTML = `
        <div class="modal-content scale-in" style="max-width:400px; padding:0; border-radius:20px; overflow:hidden; border:none; background:#f0f2f5;">
            ...
            <div style="background:white; padding:15px; display:flex; gap:10px; align-items:center; border-top:1px solid #eee;">
                ...
                <div onclick="sendAdminUserRealMessage('${userId}')" style="width:40px; height:40px; background:#1a2a3a; border-radius:50%; display:flex; align-items:center; justify-content:center; color:white; cursor:pointer; box-shadow:0 4px 10px rgba(26, 42, 58, 0.2);">
                    <i class="fas fa-paper-plane"></i>
                </div>
            </div>
        </div>
...
window.sendAdminUserRealMessage = (userId) => {
